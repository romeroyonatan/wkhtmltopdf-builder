...
ENV QMAKE_CXXFLAGS="-std=gnu++98 -I/usr/local/ssl/include"
# Link openssl statically
ENV QMAKE_LFLAGS="-static-libgcc -L/usr/local/ssl/lib"
...
COPY --from=build_openssl /usr/local/ssl /usr/local/ssl/
RUN cd wkhtmltopdf-${WKHTMLTOPDF_VERSION}/ && \
    scripts/build.py posix-local && \
    cp src/wkhtmltopdf-${WKHTMLTOPDF_VERSION}/static-build/wkhtmltox-${WKHTMLTOPDF_VERSION}_local-*.tar.xz /wkhtmltox-${WKHTMLTOPDF_VERSION}.tar.xz
